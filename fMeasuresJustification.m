...
for j0=1:m0                                           % for each current subdomain
    ...
    if ~isempty(core_measures_index{j0})
        minchoose = 1;
        count = 1;
        index = find(temp);
...
%             datsub = [datsub_core datsub_cur];
    ...
    includedsSaved{j0} = includeSavedtemp;
...
for j0=1:m0  
    ...
    indexorigin = [core_measures_index{j0}  non_core_index{j0}'];
    [orderedindex, order] = sort(indexorigin);
    temp(1:length(indexorigin),:) = temp(order,:);
    includedsSaved{j0} = temp;
    
    temp = SignsSaved{j0};
    temp = [direc0(:,core_measures_index{j0})' * ones(1,numcol) ;temp];
    temp(1:length(indexorigin),:) = temp(order,:);
    SignsSaved{j0} = temp;
