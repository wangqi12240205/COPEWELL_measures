    ...
    for j0=1:m0                                              % for each current subdomain
        ...
        if n_DS0(j0)>2                                       % if at least two measures
            ...
            for j0sub=1:n_DS0(j0)                            % subtract current measure
                if (Signs(Is0(j0sub)) == '-')
                    Is0sub = setdiff(Is0sub,Is0(j0sub));            % get reduced set
                    CAsubstract = fCAlpha([Dat0(:,Is0sub) Dat(:,I_DS0_add)],[Direc0max(Is0sub) sgns_add]);
                    if (length(I_DS0_match_temp) == 0)
                        fprintf('\nSubstract current measure ID %d: %5.2f\nNothing to be added',current_ID(Is0(j0sub)),CAsubstract);
                    else
                        ...
                        CAlast = CAmaxs(j0sub);
                        fprintf('\nSubstract current measure ID %d: %5.2f\nAdd original measure No.%d: %5.2f',current_ID(Is0(j0sub)),CAsubstract,Imaxs(j0sub),CAmaxs(j0sub));
                    end
                end
            end
            ...
            ID = [Is0sub; I_DS0_add];
            mea_domain = [Dat0(:,Is0sub) Dat(:,I_DS0_add)];
            [~,num] = size(mea_domain);
            % covariance
            ...
            title([u_DS0(j0,:) ', Cronbuchs'' alpha =' num2str(round(CAlast,3))],'Fontsize',18) % title
